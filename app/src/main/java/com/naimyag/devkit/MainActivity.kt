...
import com.naimyag.devkit.databinding.ActivityMainBinding
                ...
                when (resp) {
                    ...
                    is Result.Error -> {
                        when (resp.exception.response()!!.code()) {
                            ...
                            402 -> {
                                printEx("onCreate() getBesinler 402 ex:", resp.exception)
                            }
                            403 -> {
                                printEx("onCreate() getBesinler 403 ex:", resp.exception)
                            }
                            404 -> {
                                printEx("onCreate() getBesinler 404 ex:", resp.exception)
                            }
                            else -> {
